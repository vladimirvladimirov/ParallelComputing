...
/**
 * Generates test cases for the Traveling Salesman problem. The input data consists of only the graph definition in
 * terms of number of nodes, number of edges and the edges themself.
 */
public class TravelingSalesmanTestGenerator {
    ...
    public TravelingSalesmanTestGenerator(Map<GraphConstant, Integer> graphProperties, String inputFilename, String solutionFilename) throws IOException {
        ...
        for(firstEnd = firstIndex; firstEnd < firstIndex + nodeNumber; firstEnd++) {
            for(secondEnd = firstEnd + 1; secondEnd <firstIndex + nodeNumber; secondEnd++) {
                if (!graph.hasEdge(firstEnd,secondEnd)) {
                    int weight = Utils.getIntInRange(maxEdge, maxEdgeWeight);
                    if (weight == 0) {
